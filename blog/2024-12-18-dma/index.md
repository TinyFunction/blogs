...
DMA 每次传输一个字节，但它会间歇性地请求总线控制权。这种模式允许 CPU 在 DMA 间歇期继续执行任务。适合需要同时运行 CPU 和 DMA 的场景。
...
| 位号 | 名称 | 描述 |
| :--- | :--- | :--- |
| 7-6 | 通道选择位（Channel Select） | 选择要设置的 DMA 通道（`00`=通道 0，`01`=通道 1，`10`=通道 2，`11`=通道 3）。 |
| 5-4 | 传输方向 | `00` = 内存到内存，`01` = 从设备到内存，`10` = 从内存到设备。 |
| 3-2 | 传输模式 | `00` = 验证模式，`01` = 单字节传输，`10` = 块传输，`11` = 需求模式。 |
| 1-0 | 保留位 | 未定义用途，要求始终设置为 `0`。 |
...
### 寄存器与端口映射一览
| 寄存器名称 | 通道编号 | 地址（主 DMA） | 地址（高 DMA，64KB+ 支持） |
| --- | --- | --- | --- |
