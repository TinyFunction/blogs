...
注意📢：这篇文章的内容<font style="color:#DF2A3F;">不敢保证百分百正确</font>，尤其是通信步骤的部分！因为本文最后的例程我虽然写出来了，但一直没有跑通！这有可能是 bochs 的问题，也有可能是程序中哪个部分不符合 FDC 的控制协议！
...
+ 磁道号 $ \text{Cylinder} = \text{LBA} / (\text{Heads} \times \text{SectorsPerTrack}) $
+ 磁头号 $ \text{Head} = (\text{LBA} / \text{SectorsPerTrack}) \mod \text{Heads} $
+ 扇区号 $ \text{Sector} = (\text{LBA} \mod \text{SectorsPerTrack}) + 1 $
